...
const fetchKPIs = async (req, res, next) => {
  try {
    ...
    const rules = [
      ...
      { section: 'general', collection: 'requests', filter: { $or: [{ status: 'paid' }, { status: 'delivered' }, { status: 'preparing' }], type: { $regex : ".*Batiment.*" }}, id: 'nb_paid_building_requests', type: 'count', label: 'Batiment', model: requestModel },
      ...
      { section: 'month', collection: 'requests', filter: { $or: [{ status: 'paid' }, { status: 'delivered' }, { status: 'preparing' }], type: { $regex : ".*Batiment.*" }, createdAt: { $gte: monthFirstDay, $lte: monthLastDay } }, id: 'month_nb_paid_building_requests', type: 'count', label: 'Batiment', model: requestModel },
